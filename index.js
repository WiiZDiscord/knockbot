...
bot.on('guildMemberAdd', member => {
    bot.channels.get("451174045203955723").send("Bienvenue sur __Test Bot__ " + member.user.username);
    //logs
...
bot.on('guildMemberRemove', member => {
	bot.channels.get('451174045203955723').send(`__Aurevoir__ ${member.user.username} :cry:`)
})
...
bot.on('message', message => {
	...
	var help_embed = new Discord.RichEmbed()
		.setColor('#F4FF3A')
		.addField("Infos", "p.info Affiche les infos du bot\np.si Affiche les infos du serveu\np.wiiz Lien vers le serveur discord de WiiZ")
...
bot.on('message', message => {
	if(message.content === 'mdr'){
		message.reply("Je suis du même avis que Dyno :expressionless:")
	}
})
...
bot.on('message', message => {
	if(message.content === 'xD'){
		message.reply("Arrêtez de dire **xD** Ca ne ce dit PLUS en 2018 :ragequit: ")
	}
})

bot.on('message', message => {
	if(message.content === 'rt'){
		message.reply("Rt si c'est triste :'(")
	}
})

bot.on('message', message => {
	if(message.content === prefix + 'raid'){
		message.reply("Tu as cru que j'étais un bot de raid ? :ragequit:")
	}
})

bot.on('message', message => {
	if(message.content === 'rip'){
		message.reply("Tu resteras toujours dans nos coeurs :cry ")
	}
})

bot.on('message', message => {
	if(message.content === 'gn'){
		message.channel.send('Bonne nuit mon grand !')
	}
})
...
bot.on("message", message => {
	if(message.content === prefix + "wiiz"){
	var help_embed = new Discord.RichEmbed()
		.setColor("#4286f4")
		.setTitle("Voici un lien vers le serveur de WiiZ")
		.addField("Mes/Vos Bots", "[Lien Cliquable](https://discord.gg/SJgTZDT)")
		message.channel.sendEmbed(help_embed)
	}
})
...
bot.on('message', message => {
	...
	if(command === 'say') {
  if(!args[0]) return message.channel.send("Veuillez introduire un texte ! Ex k!say Hey, je m'appelle KnockBot, mon codeur est WiiZ");
		var help_embed = new Discord.RichEmbed()
		.setColor('RANDOM')
